...
- **Ship/PR command:** `/tyrex-ship` creates a pull request from a completed, verified, reviewed feature branch. Auto-generates PR title (conventional commit) and body from feature spec, CHANGELOG, task summaries, verification report, and review findings. Supports `--draft` and `--squash` flags. Pushes branch, creates PR via `gh` CLI (manual fallback if unavailable). Updates feature state to `shipped`, syncs to external tracker (max status: review). `/tyrex-quick` suggests ship after accept. Command count: 25 (was 24). Inspired by GSD /gsd:ship.
- **Research integrated in plan pipeline:** `/tyrex-plan` Step 2b spawns parallel researcher sub-agents (stack, approach, pitfalls, architecture) before task decomposition. Results persist in `NNN-research.md` and feed the planner as context. Configurable via `tyrex.yml` `workflow.research_before_plan` (default: true). Auto-skips if recent research exists. Fallback to inline research if sub-agents unavailable. `/tyrex-research` standalone continues independently. Inspired by GSD research phase.
- **Verify/UAT as pipeline stage:** `/tyrex-verify` provides manual user acceptance testing between do and review. Extracts testable deliverables from specs/acceptance criteria, walks through one at a time (pass/fail/skip). On failure: auto-diagnosis + fix task creation (prefix `fix-`). Fix→re-verify loop (max 3 iterations). Results persist in `NNN-verify.md`. Pipeline: new→plan→do→verify→review→accept. `/tyrex-quick` includes verify automatically. Plan mode — diagnose only, never writes code. Inspired by GSD /gsd:verify-work.

- **Structured session logging:** Execution sessions (tyrex-do, tyrex-quick) generate YAML logs in `.tyrex/metrics/sessions/SESS-NNN.yml`. Shared algorithm in `templates/commands/shared/session-log.md` defines 6 hooks (session_start, wave_start, task_start, task_end, wave_end, session_end). Index in `.tyrex/metrics/index.yml` (auto-increment SESS-NNN IDs). tyrex-quick sessions include verify/review blocks. tyrex-status shows last 5 sessions. Foundation for EP-008 (BL-035 execution report, BL-036 quality scorecard, BL-037 context tracking). (ADR-022, BL-034)
- **Metrics registry:** `.tyrex/metrics/` stores session logs and index. `sessions/SESS-NNN.yml` = per-session data. `index.yml` = lightweight lookup. Data in YAML, API-ready (EP-009 direction).
- **Hook infrastructure for mechanical enforcement:** Shell scripts in `templates/hooks/` provide runtime-agnostic enforcement. Three hook runners: `pre-tool-use.sh` (Claude Code PreToolUse for Write/Edit interception), `pre-commit.sh` (git pre-commit), `commit-msg.sh` (git commit-msg). Shared library `lib/common.sh` with JSON parsing (jq + Node.js fallback), YAML reading, file classification, and validator discovery. Validators are executable scripts in `validators/` directory — auto-discovered and run by hook runners. `tyrex init` auto-installs hooks: copies scripts to `.tyrex/hooks/`, configures `.claude/settings.json` for Claude Code, creates git hook symlinks. Claude Code first, architecture is LLM-agnostic. (EP-007, BL-028)
...
| 2026-03-27 | Verify/UAT as pipeline stage (ADR-019) | `/tyrex-verify` for manual user acceptance testing between do and review. Deliverable extraction from specs, walk-through, auto-diagnosis on failure, fix task creation, re-verify loop. Pipeline: new→plan→do→verify→review→accept. `/tyrex-quick` integrates verify automatically. Plan mode. Command count: 24 (was 23). Inspired by GSD /gsd:verify-work. |
| 2026-03-27 | Research integrated in plan (ADR-020) | `/tyrex-plan` Step 2b spawns 4 parallel researchers (stack, approach, pitfalls, architecture) before task decomposition. Results in `NNN-research.md`. Config: `workflow.research_before_plan` (default: true). Auto-skip if recent. Inline fallback. `/tyrex-research` standalone unaffected. Inspired by GSD. |
| 2026-03-27 | Ship/PR command (ADR-021) | `/tyrex-ship` creates PR from completed feature. Auto-generates title + body from artifacts. `--draft` and `--squash` flags. `gh` CLI with manual fallback. Tracker sync to `review` status. `/tyrex-quick` suggests ship after accept. Pipeline: new→plan→do→verify→review→accept→ship. Command count: 25. Inspired by GSD /gsd:ship. |
| 2026-03-27 | Production-ready v2.0 initiative (ADR-016*) | 4 new epics: EP-007 (Enforcement Mecânico), EP-008 (Observabilidade), EP-009 (Data Architecture API-Ready), EP-010 (Governance Layer). Breaking change planned. Principles: data in YAML, narrative in MD with frontmatter; mechanical enforcement via hooks; single source of truth; Claude Code first, LLM-agnostic. |
| 2026-03-27 | Hook infrastructure (EP-007, BL-028) | Shell scripts in `templates/hooks/` for mechanical enforcement. Three runners (PreToolUse, pre-commit, commit-msg) + shared library + validator plugin pattern. `tyrex init` auto-installs. Claude Code first via `.claude/settings.json` hooks config. |
| 2026-03-28 | Structured session logging (ADR-022) | YAML session logs in `.tyrex/metrics/sessions/SESS-NNN.yml`. Shared algorithm with 6 hooks. Index for fast lookup. tyrex-do + tyrex-quick produce; tyrex-status consumes. First EP-008 deliverable. |
