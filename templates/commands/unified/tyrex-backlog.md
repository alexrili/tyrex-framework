...
- **`/tyrex-backlog seed "idea" --trigger "condition"`** — Plant a seed: a forward-looking idea with a trigger condition
- **`/tyrex-backlog seeds`** — List all active seeds
...
1. Check `.tyrex/backlog/` exists. If not, create directory structure:
   ...
   └── seeds/
   ```
2. Scan existing items, epics, and seeds for the session.
...
5. **Seeds section:** If `.tyrex/backlog/seeds/` has active seeds, show them after the main table:
   ```
   Seeds (active):
     SEED-001  "Consider OAuth when implementing auth"  (trigger: "auth implementation")
     SEED-002  "Add rate limiting to API"               (trigger: "API endpoints")
     ...
   ```
...
### Subcommand: `seed "idea" --trigger "condition"`

Plant a forward-looking idea with a trigger condition. Seeds differ from backlog items:
- Backlog items are manually picked for execution
- Seeds surface **automatically** when their trigger condition matches the current context

1. Auto-assign next SEED-NNN ID (sequential, never reuse dismissed IDs)
2. Present the seed for confirmation:
   ```
   New seed:
     ID: SEED-NNN
     Idea: [user's idea]
     Trigger: [user's condition]

     [1] Plant seed
     [2] Edit before planting
     [3] Cancel
   ```
3. Save to `.tyrex/backlog/seeds/SEED-NNN.yml`
4. Commit: `backlog: plant SEED-NNN — [idea summary]`
...
═══════════════════════════════════════
  ...
  ID        Idea                                    Trigger                    Status
  SEED-001  Consider OAuth for auth                 auth implementation        active
  SEED-002  Add rate limiting to API                API endpoints              active
  SEED-003  Consider caching layer                  database optimization      promoted → BL-028
  SEED-004  Add WebSocket support                   real-time features         dismissed

Summary: N seeds (A active, P promoted, D dismissed)
...
**Dismiss:** Set seed status to `dismissed`. Commit: `backlog: dismiss SEED-NNN — [idea summary]`
...
- **`/tyrex-new`:** After feature source selection (Step 0), checks active seeds against the feature description. Matching seeds are surfaced with promote/dismiss/skip options.
- **`/tyrex-plan`:** After loading context (Step 1), checks active seeds against the feature scope and task areas. Matching seeds are surfaced before task decomposition.
...
### Seed (`.tyrex/backlog/seeds/SEED-NNN.yml`)
```yaml
id: SEED-NNN
idea: "Short description of the forward-looking idea"
trigger_condition: "Context condition that should surface this seed"
status: active | promoted | dismissed
created: "YYYY-MM-DD"
promoted_to: null  # BL-NNN when promoted
origin: "manual" | "discuss session YYYY-MM-DD"
```
...
## Git Semantic Commits & Tags

After each subcommand that modifies `.tyrex/backlog/` files, auto-commit per `templates/commands/shared/git-semantic-commits.md`:

- **add:** `backlog: add BL-NNN — [item title]`
- **edit:** `backlog: edit BL-NNN — [what changed]`
- **remove/discard:** `backlog: discard BL-NNN — [item title]`
- **plan:** `backlog: replan — [summary of changes]`
- **pick:** `backlog: pick BL-NNN — [item title]`
- **ready:** `backlog: ready BL-NNN — [item title]`
- **seed plant:** `backlog: plant SEED-NNN — [idea summary]`
- **seed dismiss:** `backlog: dismiss SEED-NNN — [idea summary]`
- **seed promote:** `backlog: promote SEED-NNN → BL-NNN — [idea summary]`

Check `tyrex.yml` `git.auto_commit_state` before committing. If `off`, skip silently.

**Phase completion tag:** After any item status changes to `done` or `discarded`, scan all items in that item's phase. If ALL items in the phase are `done` or `discarded`, and `tyrex.yml` `git.auto_tag` is `true`, create tag: `tyrex-phase-N-done`.
